...
class Better_Post_Date extends Widget_Base {
	...
	public function get_title() {
		return __( 'Better Post Date', 'better-widgets' );
	}
	...
	protected function _register_controls() {
		...
		$this->start_controls_section(
			'section_content',
			[
				'label' => __( 'Data setings', 'better-widgets' ),
			]
		);
		...
		$this->add_control(
			'date_type',
			[
				'label' => __( 'Date Type', 'better-widgets' ),
				'type' => Controls_Manager::SELECT,
				'options' => [
					'publish' => __( 'Publish Date', 'better-widgets' ),
					'modified' => __( 'Last Modified Date', 'better-widgets' ),
				],
				'default' => 'publish',
			]
		);
		...
		$this->start_controls_section(
			'section_style',
			[
				'label' => __( 'Setings', 'better-widgets' ),
	...
	protected function render() {
		...
		$html = sprintf( '<%1$s class="better-widgets-date %2$s">', $settings['html_tag'], $animation_class );
		$html .= $date;
